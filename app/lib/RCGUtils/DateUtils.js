...
class DateUtils{
	toDateNormalDDMMYYYYHHMMSS(sDate){ //dd/mm/yyyy hh:mm:ss
		var arrFecha=sDate.split(" ");
		var arrFecha=arrFecha[0].split("/");
		
	    var curr_date= parseInt(arrFecha[0]);
	    var curr_month = parseInt(arrFecha[1])-1;
	    var curr_year = parseInt(arrFecha[2]);
	    var curr_hr= 0;
	    var curr_min = 0;
	    var curr_sc= 0;
		if (sDate.length>12){
	...
	formatDate(dateObj,format) {
	    ...
	    {
			log("Fecha de Envio:"+curr_year+""+curr_month+""+curr_date+""+curr_hr+""+curr_min+""+curr_sc);
	        return curr_year+""+curr_month+""+curr_date+""+curr_hr+""+curr_min+""+curr_sc;       
	    }
